...
public class Enemy02Control : MonoBehaviour
{
    ...
    IEnumerator MoveEnemyIntoScreen(GameObject enemy)
    {
        ...
        while (elapsedTime < moveDuration)
        {
            ...
            // 斜め上方向に移動（X軸とY軸の移動速度）
            enemy.transform.Translate(new Vector3(moveSpeedX, Mathf.Abs(moveSpeedY), 0) * Time.deltaTime);
    ...
    // エネミーがランダムに斜め上方向に動く処理
    IEnumerator MoveEnemy(GameObject enemy)
    {
        float randomDirectionInterval = 2f;  // ランダムに方向を変える間隔

        while (enemy != null)
        {
            // ランダムな方向（Y軸は常に上方向のみに制限）
            float randomX = Random.Range(-1f, 1f);
            float randomY = Random.Range(0f, 1f);  // Y軸は0から1の範囲に制限して上方向のみ
